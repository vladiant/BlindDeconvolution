...
// k_out = output kernel
//
// This version of the function uses spatial convolutions. Everything is
// maintained as 2D arrays
//
...
void pcg_kernel_irls_conv(const cv::Mat& k_init, const std::vector<cv::Mat>& X,
                          ...
                          const BlindDeblurContext& aContext, cv::Mat& k_out) {
  // Defaults
  if (false) {  // nargin == 3
    //		opts.lambda = 0;
    //		// PCG parameters
    ...
    //		opts.pcg_its = 100;
    //		std::cout
    //				<< "Input options not defined - really no reg/constraints on the
    //kernel?"
    //				<< std::endl;
  }
  ...
  for (unsigned int i = 0; i < X.size(); i++) {
    ...
    conv((float*)flipX[i].data, flipX[i].cols, flipX[i].rows, flipX[i].cols,
         (float*)Y[i].data, Y[i].cols, Y[i].rows, Y[i].cols,
         (float*)rhs[i].data, rhs[i].cols, rhs[i].rows, rhs[i].cols);
  ...
  for (int iter = 0; iter < 1; iter++) {
    ...
    local_cg(k_prev, X, flipX, ks, weights_l1, rhs[0], pcg_tol, pcg_its, k_out);
...
void local_cg(const cv::Mat& k, const std::vector<cv::Mat>& X,
              const std::vector<cv::Mat>& flipX, int ks,
              const cv::Mat& weights_l1, const cv::Mat& rhs, float tol,
              int max_its, cv::Mat& k_out) {
  k_out = k.clone();
  cv::Mat Ak;
...
void pcg_kernel_core_irls_conv(const cv::Mat& k, const std::vector<cv::Mat>& X,
                               const std::vector<cv::Mat>& flipX, int ks,
                               const cv::Mat& weights_l1, cv::Mat& out) {
  ...
  for (unsigned int i = 0; i < X.size(); i++) {
    ...
    conv((float*)X[i].data, X[i].cols, X[i].rows, X[i].cols, (float*)k.data,
         k.cols, k.rows, k.cols, (float*)tmp1.data, tmp1.cols, tmp1.rows,
         tmp1.cols);
    ...
    conv((float*)flipX[i].data, flipX[i].cols, flipX[i].rows, flipX[i].cols,
         (float*)tmp1.data, tmp1.cols, tmp1.rows, tmp1.cols, (float*)tmp3.data,
         tmp3.cols, tmp3.rows, tmp3.cols);
    ...
    /*
     ...
     //				tmp1Kernel.rows, tmp1Kernel.cols, (float*) tmp3.data,
     tmp3.cols,
     //				tmp3.rows, tmp3.cols);
...
void copyImageToKernel(const float* pImagedata, int imageCols, int imageRows,
                       int imagePpln, float* pKerneldata, int kernelCols,
                       int kernelRows, int kernelPpln) {
  for (int row = 0; row < kernelRows; row++) {
    ...
    for (int col = 0; col < kernelCols; col++) {
      ...
      pKerneldata[col + row * kernelPpln] =
          pImagedata[inputCol + inputRow * imagePpln];
    }
  }
...
void copyKernelToImage(const float* pKerneldata, int kernelCols, int kernelRows,
                       int kernelPpln, float* pImagedata, int imageCols,
                       int imageRows, int imagePpln) {
  for (int row = 0; row < kernelRows; row++) {
    int inputRow;
    if (row < kernelRows / 2) {
      inputRow = imageRows + row - kernelRows / 2;
