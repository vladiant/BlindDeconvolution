...
//	cv::waitKey(0);
	...
	if (lcost.size() > totiter) {
		...
		lcost.push_back(
				(lambda / 2.0) * cv::norm(gk - f, cv::NORM_L2)
						* cv::norm(gk - f, cv::NORM_L2));
		cv::Mat gxPow;
		cv::pow(cv::abs(gx), alpha, gxPow);
		cv::Mat gyPow;
...
//			cv::waitKey(0);
			...
			for (size_t i = 0; i < wxy1s.size(); i++) {
				cv::mulSpectrums(nums[i], denom, Fgs[i],
...
void computeConstants(const cv::Mat& f, const cv::Mat& k, const cv::Mat& dx,
		const cv::Mat& dy, cv::Mat& Ktf, cv::Mat& KtK, cv::Mat& DtD,
		cv::Mat& Fdx, cv::Mat& Fdy) {
	...
	otfk = cv::Scalar(0);
	copyKernelToImage((float*) k.data, k.cols, k.rows, k.cols,
			(float*) otfk.data, otfk.cols, otfk.rows, otfk.cols);
	cv::dft(otfk, otfk);
	...
	for (size_t i = 0; i < fChannels.size(); i++) {
		...
		cv::mulSpectrums(fChannels[i], otfk, KtfChannels[i],
				cv::DFT_COMPLEX_OUTPUT, true);
	}
...
void invertFftMatrix(cv::Mat& matrixFft) {
	float realValue, imaginaryValue, sum; //temporal variables for matrix inversion
	...
	if (matrixHeight % 2 == 0) {
		float downLeftValue = ((float*) (matrixData
				+ (matrixHeight - 1) * matrixStep))[0];
		if (downLeftValue != 0) {
			((float*) (matrixData + (matrixHeight - 1) * matrixStep))[0] = 1.0
					/ downLeftValue;
		} else {
	...
	if (matrixWidth % 2 == 0) {
		...
		//set last column
		for (int row = 1; row < matrixHalfHeight; row += 2) {
			realValue = ((float*) (matrixData + row * matrixStep))[matrixWidth
					- 1];
			imaginaryValue =
					((float*) (matrixData + (row + 1) * matrixStep))[matrixWidth
							- 1];
			sum = realValue * realValue + imaginaryValue * imaginaryValue;
			if (sum != 0) {
		...
		//sets down right
		if (matrixHeight % 2 == 0) {
			float downRightValue = ((float*) (matrixData
					+ (matrixHeight - 1) * matrixStep))[matrixWidth - 1];
			if (downRightValue != 0) {
				((float*) (matrixData + (matrixHeight - 1) * matrixStep))[matrixWidth
						- 1] = 1.0 / downRightValue;
			} else {
				((float*) (matrixData + (matrixHeight - 1) * matrixStep))[matrixWidth
						- 1] = 0.0;
			}
	...
	for (int row = 0; row < matrixHeight; row++) {
		for (int col = 1; col < matrixHalfWidth; col += 2) {
			...
			sum = realValue * realValue + imaginaryValue * imaginaryValue;
			if (sum != 0) {
				((float*) (matrixData + row * matrixStep))[col] = realValue
						/ sum;
				((float*) (matrixData + row * matrixStep))[col + 1] =
						(-imaginaryValue / sum);
			} else {
