...
int ss_blind_deconv(cv::Mat& y, cv::Mat& x, cv::Mat& k, float lambda,
  ...
  cv::Mat y21 =
      y11(cv::Rect(khs, khs, y11.cols - 2 * khs - 1, y11.rows - 2 * khs - 1))
          .clone();
  cv::Mat y22 =
      y12(cv::Rect(khs, khs, y12.cols - 2 * khs - 1, y12.rows - 2 * khs - 1))
          .clone();
  ...
  lcost.push_back((lambda / 2.0) *
                  (normTmp11 * normTmp11 + normTmp12 * normTmp12));

  pcost.push_back(cv::norm(x11, cv::NORM_L1) / cv::norm(x11, cv::NORM_L2) +
  ...
  for (int iter = 0; iter < opts.xk_iter; iter++) {
    ...
    while (delta > 1e-4) {
      ...
      for (int out_iter = 0; out_iter < opts.x_out_iter; out_iter++) {
        ...
        // TODO: Debug print
        //				std::cout << "out_iter: " << out_iter <<
        //std::endl; 				std::cout << "normx1: " << normx1 << std::endl; 				std::cout
        //<< "normx2: " << normx2 << std::endl; 				std::cout << "beta1: " << beta1
        //<< std::endl; 				std::cout << "beta2: " << beta2 << std::endl;
        ...
        for (int inn_iter = 1; inn_iter < opts.x_in_iter; inn_iter++) {
          ...
          if (totiter < pcost.size() - 1) {
            pcost[totiter] = cv::norm(x11, cv::NORM_L1) / cv::norm(x11) +
                             cv::norm(x12, cv::NORM_L1) / cv::norm(x12);
          } else {
            pcost.push_back(cv::norm(x11, cv::NORM_L1) / cv::norm(x11) +
                            cv::norm(x12, cv::NORM_L1) / cv::norm(x12));
          }  // if (totiter < pcost.size() - 1) ..

          // TODO: Debug print
          //					std::cout << "lcost[" << totiter << "]: " <<
          //lcost[totiter]
          //							<< std::endl;
          //					std::cout << "pcost[" << totiter << "]: " <<
          //pcost[totiter]
          //							<< std::endl;

          // TODO: Debug print
          //					cv::namedWindow("test x11",
          //cv::WINDOW_NORMAL); 					cv::namedWindow("test x12", cv::WINDOW_NORMAL);
      ...
      // TODO: Debug print
      //			std::cout << "cost_after_x: " << cost_after_x <<
      //std::endl;
