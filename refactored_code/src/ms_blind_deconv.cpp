...
void ms_blind_deconv(cv::Mat& blurredImage, const BlindDeblurOptions& opts,
  ...
  if (blurredImage.empty()) {
    ...
    switch (blurredImage.channels()) {
      ...
      case 3:
        ...
        break;
      default:
        std::cerr << "Algorithm not implemented for " << blurredImage.channels()
                  << " !" << std::endl;
        return;
    }
  }
  ...
  // image
  cv::Mat y1;
  cv::resize(y, y1, cv::Size(), opts.prescale, opts.prescale, cv::INTER_LINEAR);
  y = y1;
  ...
  int minsize =
      std::max(3.0, 2.0 * floor(((opts.kernel_size - 1.0) / 16.0)) + 1.0);
  ...
  for (int s = 0; s < num_scales; s++) {
    ...
    if (s == 0) {
      ...
      cv::resize(tmp, ks[s], cv::Size(k1, k2), 0.0, 0.0, cv::INTER_LINEAR);
    ...
    if (s == 0) {
      ...
      if (error_flag != 0) {
        ...
        cv::Mat tmp1_up;
        cv::Mat tmp2_up;
        cv::resize(tmp1, tmp1_up, cv::Size(c, r), 0.0, 0.0, cv::INTER_LINEAR);
        cv::resize(tmp2, tmp2_up, cv::Size(c, r), 0.0, 0.0, cv::INTER_LINEAR);
    ...
    error_flag =
        ss_blind_deconv(g, ls[s], ks[s], aContext.lambda, opts, aContext);
    ...
    if (error_flag < 0) {
      ...
      std::cout << "Bad error - just set output to delta kernel and return"
                << std::endl;
    }
    ...
    //
    //		center_kernel_separate(tmp1, tmp2, ks[s], tmp1_shifted,
    //tmp2_shifted, 				ks[s]);
    //
  ...
  tmp(cv::Rect(kernelImage.cols, kernelImage.rows, ycbcr.cols, ycbcr.rows))
      .copyTo(deblurredImage);
...
void init_kernel(int minsize, cv::Mat& k) {
  ...
  k = cv::Scalar(0);
  k(cv::Rect((minsize - 1) / 2, (minsize - 1) / 2, 1, 1)) =
      cv::Scalar(1.0 / 2.0);
}

// Center the kernel by translation so that boundary issues are mitigated.
// Additionally, if one shifts the kernel the the image must also be shifted in
...
void center_kernel_separate(const cv::Mat& x, const cv::Mat& y,
  ...
  cv::Mat shift_kernel(abs(offset_y * 2) + 1, abs(offset_x * 2) + 1, CV_32FC1);
  shift_kernel = cv::Scalar(0.0);
  shift_kernel.at<float>(
      cv::Point(abs(offset_x) + offset_x, abs(offset_y) + offset_y)) = 1.0;
  ...
  for (int row = 0; row < k.rows; row++) {
    ...
    for (int col = 0; col < k.cols; col++) {
      ...
      kshift.at<float>(cv::Point(shiftedCol, shifterRow)) =
          k.at<float>(cv::Point(col, row));
    }
  }
  ...
  for (int row = 0; row < x.rows; row++) {
    ...
    for (int col = 0; col < x.cols; col++) {
      ...
      xshift.at<float>(cv::Point(shiftedCol, shifterRow)) =
          x.at<float>(cv::Point(col, row));
      yshift.at<float>(cv::Point(shiftedCol, shifterRow)) =
          y.at<float>(cv::Point(col, row));
    }
  }
