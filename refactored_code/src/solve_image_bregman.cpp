...
float BregmanImageSolver::WeightCalcAlpha(float v, float beta) {
	...
	if (index < 0) {
		...
		float indexValue = 32.0 * (index - WEIGHT_LUT_SIZE / 2.0)
				/ WEIGHT_LUT_SIZE;
		float interpolationWeight = v - indexValue;

		float value = weightLut[index]
				+ interpolationWeight
						* (weightLut[index + 1] - weightLut[index]);
		return value;
	}
}
	...
	for (int channel = 0; channel < imageChannels; channel++) {
		for (int row = 0; row < imageHeight; row++) {
			for (int col = 0; col < imageWidth; col++) {
				float value = inputImageData[imageChannels
						* (col + row * imageWidth) + channel];
				value = WeightCalcAlpha(value, beta);
				outputImageData[imageChannels * (col + row * imageWidth)
						+ channel] = value;
			}
		}
	}
